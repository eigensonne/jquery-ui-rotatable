...
$.widget('ui.rotatable', $.ui.mouse, {
	...
	perform: function (element, angle) {
		...
		var newTransform =  '--rotation:' + angle + 'deg; ';
		element.attr('data-rotation', angle);
		...
		element.attr('style', newTransform).addClass('rotated');
		return angle;
	},
	...
	_destroy: function () {
		...
		//console.log('%c Handle amount: %s', sr.logState.warn, this.element.find('.' + this.options.rotClass));

		this.element.find('.' + this.options.rotClass).remove();
	...
	_placeHandle: function () {
		...
		var jqHandle = this.element.find('.' + this.options.rotClass);
		if (jqHandle.length < 1) {
			jqHandle = $(o.handleElementSelector);
			jqHandle.addClass(this.options.rotClass);
		...
		if (jqHandle.width() < 1) {
			jqHandle.width(o.handle.width);
		}
		if (jqHandle.height() < 1) {
			jqHandle.height(o.handle.height);
		}
	...
	_getJqHandle: function () {
		var o = this.options;
		if (o.handle.set) {
			return this.element.find('.' + this.options.rotClass);
		}
		return this.element;
	},
	...
	_mouseCapture: function (event) { // event handler
		...
		if (o.handle.set) {
			...
			for (var i = 0; i < jqHandle.length; i++) {
				// added a 'for' loop to get all handles and changed condition
				// from 'event.target !== $jqHandle[0]'
				if (event.target === $(event.target).hasClass(o.rotClass)) {
					return false;
				}
			}
		}
